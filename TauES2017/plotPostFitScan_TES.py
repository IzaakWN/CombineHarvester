...
from plotParabola_TES import measureTES, plotMeasurements
...
# parameter_dict = {
#     "eff_t", "zjXsec", "wNorm", 
#     "shape_jetTauFake", "rate_jetTauFake", "ttjXsec",
#     "zjXsec", "ttjXsec", "stXsec",
#     "shape_jetTauFake",
#     "rate_jetTauFake",
#     "shape_dy",
#     "shape_m",
...
def plotCorrelation(channel,var,DM,*parameters,**kwargs):
    ...
    filename    = '%s/higgsCombine.%s_%s-%s%s-13TeV.MultiDimFit.mH90.root'%(DIR,channel,var,DM,tag)
    ...
    canvasname = "%s/postfit-correlation_%s_%s%s%s"%(PLOTS_DIR,var,DM,tag,plotlabel)
...
def plotPostFitValues(channel,var,DM,*parameters,**kwargs):
    ...
    filename    = '%s/higgsCombine.%s_%s-%s%s-13TeV.MultiDimFit.mH90.root'%(DIR,channel,var,DM,tag)
    filenamesFD = '%s/fitDiagnostics.%s_%s-%s%s-13TeV_TES*p*.root'%(DIR,channel,var,DM,tag)
    print '>>>   file "%s"'%(filename)
    ...
    if not name:
      name = formatParameter(parameters[0])
    if len(parameters)>1:
      name = "comparison_%s"%(name) #re.sub(r"bin_\d+","bin",name)
    canvasname = "%s/postfit-%s_%s_%s%s%s"%(PLOTS_DIR,name,var,DM,tag,plotlabel)
    canvas.SaveAs(canvasname+".png")
    if args.pdf: canvas.SaveAs(canvasname+".pdf")
    canvas.Close()
...
def formatParameter(param):
    ...
    param = param.replace("CMS_","").replace("ttbar_","").replace("ztt_","").replace("_mt_13TeV","").replace("_13TeV","")
    return param
...
def getBBBList(channel,var,DM,process,**kwargs):
...
def main():
    ...
    nuisances = [ "CMS_eff_t", "CMS_ztt_zjXsec_13TeV", #"CMS_ztt_wNorm_13TeV", 
          ...
          for DM in DMs:
            ...
            for process in procsBBB:
              ...
              for bbblist in bbblists:
                plotPostFitValues(channel,var,DM,*bbblist,tag=tag,compareFD=False,title=title)
            
            # CORRELATION
            correlate = ['tes'] + compare[0] + getBBBList(channel,var,DM,'ZTT',tag=tag)
            plotCorrelation(channel,var,DM,correlate,tag=tag,title=title)
