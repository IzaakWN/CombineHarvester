...
def renameHistogramsInDir(dir,histnames,**kwargs):
    ...
    deleteOld   = kwargs.get( 'deleteOld',  True    )
    cat         = dir.GetName()
    DM          = re.findall(r"DM\d+",dir.GetName())
      ...
      for key in dir.GetListOfKeys():
        if gROOT.GetClass(key.GetClassName()).InheritsFrom('TH1'):
          ...
          for find, replace in patterns:
            replace = replace.replace('$CAT',cat)
...
def main():
    ...
    #]
    filenames = glob.glob('%s/ztt*.root'%IN_DIR)
    ...
    # RENAME
    patterns = [
      #("jer_mt", "jer"), ("jes_mt", "jes"), ("jer_em", "jer"), ("jes_em", "jes")
      ("shape_mTauFake","shape_mTauFake_$CAT"),
    ]
    for filename in filenames:
...
#     for filename in filenames:
#       filename = "input/%s"%(filename)
#       rebinHistograms(filename,None,85,tag="_85",force=force)
